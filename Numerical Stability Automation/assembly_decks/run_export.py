...
# --- Step 2: Generate Consolidated Master Decks ---
pch_files = sorted(glob.glob("shell_t_*.pch"))
...
for pch_file in pch_files:
    ...
    clean = base.replace("_negative", "").replace("_ax", "")
    
    # Extract the raw thickness value from the filename string
    raw_val = float(clean.split("shell_t_")[1].split("_")[0])
    
    # Keep your exact downstream naming math so nothing breaks
    delta = (raw_val - 100.0) if raw_val >= 100.0 else raw_val
    target_T = (100.0 - delta) if is_neg else (100.0 + delta)
    
    # --- FIXED OFFSET LOGIC (OUTWARD NORMALS) ---
    # Shift distance relative to the baseline surface (modeled at 50.0mm half-height)
    surface_delta = (target_T / 2.0) - 50.0
    
    # Since both normals face OUT, moving OUT (expanding) is always POSITIVE for both.
    # Moving IN (shrinking) is always NEGATIVE for both.
    top_zoffs = surface_delta  
    bot_zoffs = surface_delta  
    # ---------------------------
